...
**Timestamps:** All event start/end times and task datetime values are displayed in local timezone as `M/D h:MM AM/PM` (e.g., `3/11 10:30 AM`). Uses the container's `TZ` environment variable. Dates (`scheduled_date`) remain as `YYYY-MM-DD`.
