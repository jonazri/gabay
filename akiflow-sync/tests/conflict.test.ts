...
describe('resolveAndUpsert — entity-level LWW', () => {
  ...
  it('overwrites local when remote global_updated_at is newer', () => {
    db.prepare(
      'INSERT INTO tasks (id, data, global_updated_at) VALUES (?, ?, ?)',
    ).run(
      'task-1',
      JSON.stringify({
        id: 'task-1',
  ...
  it('keeps local when remote global_updated_at is older', () => {
    db.prepare(
      ...
      JSON.stringify({
        id: 'task-1',
        title: 'Local winner',
        global_updated_at: '2026-01-03T00:00:00Z',
      }),
  ...
  it('keeps local when timestamps are equal', () => {
    ...
    resolveAndUpsert(
      ...
      {
        id: 'task-1',
        title: 'Remote same ts',
        global_updated_at: ts,
      },
    );
  ...
  it('works for non-task entities (labels, events) without field-level logic', () => {
    ...
    const row = db
      .prepare('SELECT data FROM labels WHERE id = ?')
      .get('label-1') as any;
