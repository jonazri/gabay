...
describe('syncV3Entity', () => {
  ...
  it('uses updatedAfter and with_deleted=true on subsequent syncs', async () => {
    // Seed a sync state
    db.prepare(
      'INSERT INTO v3_sync_state (entity, last_sync_at) VALUES (?, ?)',
    ).run('events', 1700000000000);
  ...
  it('follows next_page_url cursor', async () => {
    ...
    expect(mockFetchWithAuth.mock.calls[1][0]).toBe(
      'https://api.akiflow.com/v3/events?cursor=abc',
    );
  });
  ...
  it('saves max updated_at as last_sync_at', async () => {
    ...
    expect(getV3SyncState(db, 'events')).toBe(
  ...
  it('throws on non-ok response', async () => {
    ...
    await expect(syncV3Entity(db, 'events', auth)).rejects.toThrow(
      'V3 sync events failed: 403',
    );
  });
});
